/*
╔══════════════════════════════════════════════════
║【RunAny】一劳永逸的快速启动工具 v5.3.1 @2018.05.15
...
global RunAny_update_version:="5.3.1"
global RunAny_update_time:="2018.05.15"
Gosub,Var_Set       ;~参数初始化
		...
		try{
			...
			if(InStr(A_LoopField,"-")=1){
				...
				if(menuItem){
					...
					MenuObjTree%TREENO%[(menuRootFn[menuLevel])].Push(A_LoopField)
				}
				continue
			}
			;~添加到分类目录程序全数据
